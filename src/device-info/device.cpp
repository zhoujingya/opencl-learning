...
int main(void) {
  ...
  for (int i = 0; i < num_platforms; i++) {
    ...
    err = clGetPlatformInfo(platform[i], CL_PLATFORM_VERSION, sizeof(string),
                            &string, NULL);
    ...
    err = clGetDeviceIDs(platform[i], CL_DEVICE_TYPE_ALL, num_devices, device,
                         NULL);
    ...
    for (int j = 0; j < num_devices; j++) {
      ...
      // Get device name
      err = clGetDeviceInfo(device[j], CL_DEVICE_NAME, sizeof(string), &string,
                            NULL);
      ...
      err = clGetDeviceInfo(device[j], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                            sizeof(cl_uint), &num1, NULL);
      checkError(err, "Getting device dimension");
      printf("\t\tDevice dimension: %d\n", num1);
